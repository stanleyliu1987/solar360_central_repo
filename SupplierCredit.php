	...
	/*Now insert the credit note into the SuppTrans table*/
		...
		foreach ($_SESSION['SuppTrans']->GRNs as $EnteredGRN){
			...
			$Result = DB_query($SQL, $db, $ErrMsg, $DbgMsg, True);
                        ...
                        /* Insert Debit Note Details into podebitnotedetails table */
                       $SQL = "INSERT INTO podebitnotedetails (podetailsref,
												debitnoteref,
												debititemqty)
									VALUES ('" . $EnteredGRN->PODetailItem . "',
											'" . $CreditNoteNo . "',
											'" . $EnteredGRN->This_QuantityInv . "')";
					
			$ErrMsg =_('CRITICAL ERROR') . '! ' . _('NOTE DOWN THIS ERROR AND SEEK ASSISTANCE') . ': ' . _('The PO debit note details records could not be inserted because');
			$DbgMsg = _('The following SQL to insert the PO debit note details record was used:');
 			$Result = DB_query($SQL,$db,$ErrMsg,$DbgMsg,true);
                       /* End Insert Debit Note Details into podebitnotedetails table */
