...
/* finally write the order header to the database and then the order line details */
	...
	$ConfDate = FormatDateforSQL($_SESSION['Items'.$identifier]->ConfirmedDate);
		...
		//echo '<br><b>' . _('The Invoice Number entered') . ' <U>' . _('MUST') . '</U> ' . _('be numeric') . '.</b><br>';
                unset ($OrderNumber);
                prnMsg( _('The Invoice Number entered must be numeric'),'error');
		
	} 
        UpdateTransNo(30,$db,$OrderNo);
	...
	foreach ($_SESSION['Items'.$identifier]->LineItems as $StockItem) {
		...
		$Ins_LineItemResult = DB_query($LineItemsSQL,$db,$ErrMsg,$DbgMsg,true);
                ...
                $LineFreightCostSQL = "INSERT INTO freightcostevaluation  ( linenumber,itemcode,
					...
					'" . $OrderNo . "',
					'" . $StockItem->Quantity . "',
					'" . $_SESSION['Items'.$identifier]->LineItems[$StockItem->LineNumber]->ItemHeight . "',
					'" . $_SESSION['Items'.$identifier]->LineItems[$StockItem->LineNumber]->ItemWidth . "',
					'" . $_SESSION['Items'.$identifier]->LineItems[$StockItem->LineNumber]->ItemLength . "',
					'" . $_SESSION['Items'.$identifier]->LineItems[$StockItem->LineNumber]->ItemVolume . "',
                                        '" . $_SESSION['Items'.$identifier]->LineItems[$StockItem->LineNumber]->ItemWeight . "',
                                        '" . $_SESSION['Items'.$identifier]->LineItems[$StockItem->LineNumber]->ItemChargeWeight . "',    
                                        '" . $_SESSION['Items'.$identifier]->LineItems[$StockItem->LineNumber]->ItemShipper . "',
                                        '" . $_SESSION['Items'.$identifier]->LineItems[$StockItem->LineNumber]->ItemServiceType . "',    
                                        '" . $_SESSION['Items'.$identifier]->LineItems[$StockItem->LineNumber]->FreightCost . "',    
                                        '" . $_SESSION['Items'.$identifier]->LineItems[$StockItem->LineNumber]->ItemPrefSupp . "', 
                                        '" . $_SESSION['Items'.$identifier]->LineItems[$StockItem->LineNumber]->ItemSuppWare . "',     
                                        '" . $_SESSION['Items'.$identifier]->LineItems[$StockItem->LineNumber]->ItemFreComment . "'   
                                        )";
...
/* update the order header then update the old order line details and insert the new lines */
	...
	foreach ($_SESSION['Items'.$identifier]->LineItems as $StockItem) {
		...
		$Upd_LineItemResult = DB_query($LineItemsSQL,$db,$ErrMsg,$DbgMsg,true);
               ...
               $LineItemsSQL = "UPDATE freightcostevaluation SET quantity='"  . $StockItem->Quantity . "',
													height='" . $_SESSION['Items'.$identifier]->LineItems[$StockItem->LineNumber]->ItemHeight . "',
													width='" .  $_SESSION['Items'.$identifier]->LineItems[$StockItem->LineNumber]->ItemWidth . "',
													length='" . $_SESSION['Items'.$identifier]->LineItems[$StockItem->LineNumber]->ItemLength. "',
                                                                                                        cube='" .   $_SESSION['Items'.$identifier]->LineItems[$StockItem->LineNumber]->ItemVolume. "',
                                                                                                        weight='" . $_SESSION['Items'.$identifier]->LineItems[$StockItem->LineNumber]->ItemWeight. "',
                                                                                                        chargeweight='" . $_SESSION['Items'.$identifier]->LineItems[$StockItem->LineNumber]->ItemChargeWeight. "',    
                                                                                                        shipper='" .$_SESSION['Items'.$identifier]->LineItems[$StockItem->LineNumber]->ItemShipper. "',
                                                                                                        servicetype='" .$_SESSION['Items'.$identifier]->LineItems[$StockItem->LineNumber]->ItemServiceType. "',    
                                                                                                        freightamount ='" . $_SESSION['Items'.$identifier]->LineItems[$StockItem->LineNumber]->FreightCost . "',
                                                                                                        prefsupplier= '" . $_SESSION['Items'.$identifier]->LineItems[$StockItem->LineNumber]->ItemPrefSupp . "', 
                                                                                                        suppwarehouse='" . $_SESSION['Items'.$identifier]->LineItems[$StockItem->LineNumber]->ItemSuppWare . "', 
                                                                                                        comment='" . $_SESSION['Items'.$identifier]->LineItems[$StockItem->LineNumber]->ItemFreComment . "' 
						WHERE salesorder='" . $_SESSION['ExistingOrder'.$identifier]. "'
						AND itemcode='" . $StockItem->StockID  . "'
                                                AND linenumber='" . $StockItem->LineNumber . "'";
...
<tr>
	<td>'. _('Sales Order No.') .':</td>
	<td><input type=text size=20 maxlength=30 name="SalesOrderNo" ></td>
</tr>
<tr>
	<td>'. _('Deliver To') .':</td>
