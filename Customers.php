...
/* First check that all the necessary items have been setup */
	...
	if ($_SESSION['AutoDebtorNo']==0)  {
        $sql = "SELECT MAX(debtorno) FROM debtorsmaster";
	$result = DB_query($sql,$db);
	$myrow = DB_fetch_row($result);
	if (DB_num_rows($result)==1) {
	
           
            $CustAppen=intval(substr($myrow[0],1,4))+1;
           
            if($CustAppen<10 and $CustAppen>0){
              $CustID='C000'.$CustAppen;
            }
            elseif($CustAppen<100 and $CustAppen>=10){
              $CustID='C00'.$CustAppen;
            }
            elseif($CustAppen<1000 and $CustAppen>=100){
              $CustID='C0'.$CustAppen;  
            }
            else{
              $CustID='C'.$CustAppen; 
		...
		echo '<tr><td>' . _('Customer Code') . ':</td><td><input tabindex=1 type="text" name="DebtorNo" size=11 maxlength=10 value="'.$CustID.'"></td></tr>';
	}
...
// Show Sales Type drop down list
	$result=DB_query("SELECT typeabbrev, sales_type FROM salestypes order by typeabbrev",$db);
...
//DebtorNo exists - either passed when calling the form or from the form itself
	...
	echo '<input type="hidden" name="FormID" value="' . $_SESSION['FormID'] . '" />';
	echo '<table class=selection cellspacing=4>
			<tr><td valign=top><table class=selection><font size=2><b>Customer Details: </b></font><p>';
...
// Select Customer types for drop down list for SELECT/UPDATE
	...
	if (isset($_GET['Modify'])) {
            echo '</td></tr><tr><td colspan=2><font size=2><b>Customer Branch Details: </b></font><p>';
    ...
    echo '</td></tr><tr><td><font size=2><b>Customer Contact Details: </b></font><p>';
        /*
