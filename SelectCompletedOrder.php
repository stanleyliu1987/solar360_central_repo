...
/*show a table of the orders returned by the SQL */
	...
	$tableheader = '<tr><th>' . _('Order') . ' #</th>
						...
						<th>' . _('Delivery To') . '</th>
						<th>' . _('Order Total Inc GST') . '</th>
                                                <th>' . _('Invoice Status') . '</th>
                                                <th>' . _('Balance Due') . '</th></tr>';
	...
	while ($myrow=DB_fetch_array($SalesOrdersResult)) {
		...
		$FormatedOrderDate = ConvertSQLDate($myrow['orddate']);
		   ...
		   $sqlAP = "SELECT debtortrans.alloc, 
				...
				AND debtortrans.order_='" . $myrow['orderno'] ."'";
        ...
        $FormatedOrderValue = number_format($myrow['ordervalue']+$myrowAP['ovgst']+$myrowAP['ovfreight'],2);
        $FormatedBalanceDue = number_format($myrow['ordervalue']+$myrowAP['ovgst']+$myrowAP['ovfreight']-$myrowAP['alloc'],2);
    
                }
                else{
                   // $status='outstanding';
                    $viewOutStand=$rootpath . '/SelectSalesOrder.php';
                    $status = '<a href="'.$viewOutStand.'">outstanding</a>';
                    $FormatedOrderValue = '';
                    $FormatedBalanceDue='';
                }
