...
if (isset($_POST['submit'])) {
	...
	if (!isset($_POST['Description']) or strlen($_POST['Description']) > 150 OR strlen($_POST['Description'])==0) {
		$InputError = 1;
		prnMsg (_('The stock item description must be entered and be 150 characters or less long') . '. ' . _('It cannot be a zero length string either') . ' - ' . _('a description is required'),'error');
  ...
  /* 03042014 by Stan Customize the validation of measurements */
        if (!is_numeric($_POST['ULEN'])) {
		$InputError = 1;
		prnMsg(_('The length of the unit item must be numeric'),'error');
		$Errors[$i] = 'ULEN';
		$i++;
	}
	if ($_POST['ULEN']<0) {
		$InputError = 1;
		prnMsg(_('The length of the unit item must be a positive number'),'error');
		$Errors[$i] = 'ULEN';
		$i++;
	}
       if (!is_numeric($_POST['UWID'])) {
		$InputError = 1;
		prnMsg(_('The width of the unit item must be numeric'),'error');
		$Errors[$i] = 'UWID';
		$i++;
	}
	if ($_POST['UWID']<0) {
		$InputError = 1;
		prnMsg(_('The width of the unit item must be a positive number'),'error');
		$Errors[$i] = 'UWID';
		$i++;
	}
        if (!is_numeric($_POST['UHEI'])) {
		$InputError = 1;
		prnMsg(_('The height of the unit item must be numeric'),'error');
		$Errors[$i] = 'UHEI';
		$i++;
	}
	if ($_POST['UHEI']<0) {
			...
			*/
			...
			if ($InputError == 0){
				...
				$result = DB_query($sql,$db,$ErrMsg,$DbgMsg);
                                /* 04042014 by Stan Customize update mesurments data */
  				$sql = "UPDATE stockmeasurement
						SET     unitlength='" . $_POST['ULEN'] . "',
							unitwidth='" . $_POST['UWID'] . "',
							unitheight='" . $_POST['UHEI'] . "'
					WHERE stockid='".$StockID."'";
				$ErrMsg = _('The stock item measurements could not be updated because');
				$DbgMsg = _('The SQL that was used to update the stock item measurements and failed was');
				$result = DB_query($sql,$db,$ErrMsg,$DbgMsg);                              
			...
			if (DB_num_rows($result)==1){
				...
				if (DB_error_no($db) ==0) {
					...
					$InsResult = DB_query($sql,$db,$ErrMsg,$DbgMsg);
                                       ...
                                       /* 03042014 by Stan Insert into Stock mesurements table */
					...
					$ErrMsg =  _('The measurements for the item') . ' ' . $StockID .  ' ' . _('could not be added because');
					$DbgMsg = _('The SQL that was used to add the item failed was');
					$InMeaResult = DB_query($sql,$db,$ErrMsg,$DbgMsg);                                      
	...
	if ($CancelDelete==0) {
		$result = DB_Txn_Begin($db);
			...
			$result=DB_query($sql,$db, _('Could not delete the item record'),'',true);
                        /* 04042014 by Stan Customize delete stockmeasurements */
                        $sql="DELETE FROM stockmeasurement WHERE stockid='".$StockID."'";
			$result=DB_query($sql,$db, _('Could not delete the item measurement record'),'',true);
                        /* end of customization */

		$result = DB_Txn_Commit($db);

		prnMsg(_('Deleted the stock master record for') . ' ' . $StockID . '....' .
                '<br /> . . . .' . _('and stock measurements that may have been set up for the part') .        
...
/*If the page was called without $StockID passed to page then assume a new stock item is to be entered show a form with a part Code field other wise the form showing the fields with the existing entries against the part will show for editing with only a hidden StockID field. New is set to flag that the page may have called itself and still be entering a new part, in which case the page needs to know not to go looking up details for an existing part*/
	...
	if ($New==1) {
            //Select the last stock id
        $sql = "SELECT MAX(stockid) FROM stockmaster";
	$result = DB_query($sql,$db);
	$myrow = DB_fetch_row($result);
	if (DB_num_rows($result)==1) {
	    $StockPrefix=substr($myrow[0],0,5);
           
            $StockAppen=intval(substr($myrow[0],5,4))+1;
           
            if($StockAppen<10 and $StockAppen>0){
              $StockID=$StockPrefix.'000'.$StockAppen;
            }
            elseif($StockAppen<100 and $StockAppen>=10){
              $StockID=$StockPrefix.'00'.$StockAppen;
            }
            elseif($StockAppen<1000 and $StockAppen>=100){
              $StockID=$StockPrefix.'0'.$StockAppen;  
            }
            else{
              $StockID=$StockPrefix.$StockAppen; 
            }
	}
        else{
           $StockID ="S360-0000";
	...
	$_POST['NextSerialNo'] = $myrow['nextserialno'];
         ...
         /* 04042014 by Stan Customize retrieve measurements data */
        	$sql = "SELECT unitlength,
                               unitwidth,
                               unitheight
		FROM stockmeasurement
		WHERE stockid = '".$StockID."'";

	$result = DB_query($sql, $db);
	$myrow = DB_fetch_array($result);
        $_POST['ULEN']  = $myrow['unitlength'];
        $_POST['UWID']  = $myrow['unitwidth'];
        $_POST['UHEI']  = $myrow['unitheight'];
        /* End of modification */
...
if (!isset($_POST['UWID']) or $_POST['UWID']==''){
	$_POST['UWID']=0;
}
if (!isset($_POST['UHEI']) or $_POST['UHEI']==''){
	$_POST['UHEI']=0;
}
/*end of customization */
