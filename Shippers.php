...
if ( isset($_POST['submit']) ) {
	...
	if (strlen($_POST['ShipperName']) >40) {
        ...
        if( trim($_POST['ShipperRate'])=='' or  !is_numeric($_POST['ShipperRate'])){
            $InputError = 1;
	    prnMsg( _("The shipper's rate may be empty or rate is not a number"), 'error');
	    $Errors[$i] = 'ShipperRate';
	    $i++;
        }
	...
	if (isset($SelectedShipper) AND $InputError !=1) {
		...
		$sql = "UPDATE shippers SET shippername='" . $_POST['ShipperName'] . "', shipperrate='".$_POST['ShipperRate']."' 
                        , surchargerate='".$_POST['SurchargeRate']."', shipperwebsite='".$_POST['ShipperWebsite']."' WHERE shipper_id = '".$SelectedShipper."'";
	...
	/*SelectedShipper is null cos no item selected on first time round so must be adding a	record must be submitting new entries in the new Shipper form */
		...
		$sql = "INSERT INTO shippers (shippername,shipperrate, surchargerate,shipperwebsite) VALUES ('" . $_POST['ShipperName'] . "','".$_POST['ShipperRate']."','".$_POST['SurchargeRate']."','".$_POST['ShipperWebsite']."')";
	...
	if ($InputError !=1) {
		...
		unset($_POST['ShipperName']);
                unset($_POST['ShipperRate']);
		unset($_POST['Shipper_ID']);
                unset($_POST['ShipperWebsite']);
	}
...
if (!isset($_GET['delete'])) {
	...
	if (isset($SelectedShipper)) {
		...
		$sql = "SELECT shipper_id, shippername, shipperrate, surchargerate, shipperwebsite FROM shippers WHERE shipper_id='".$SelectedShipper."'";
		...
		$_POST['ShipperName']	= $myrow['shippername'];
                $_POST['ShipperRate']= $myrow['shipperrate'];
                $_POST['SurchargeRate']= $myrow['surchargerate'];
                $_POST['ShipperWebsite']= $myrow['shipperwebsite'];
	...
	<td><input type="Text" name="ShipperName"'. (in_array('ShipperName',$Errors) ? 'class="inputerror"' : '' ) .
        ...
        echo '<tr><td>'. _('Shipper Rate') .':</td>
	<td><input type="Text" name="ShipperRate"'. (in_array('ShipperRate',$Errors) ? 'class="inputerror"' : '' ) .
		' value="'. $_POST['ShipperRate'] .'" size=35 maxlength=40></td></tr>';
        
        echo '<tr><td>'. _('Surcharge Rate (%)') .':</td>
	<td><input type="Text" name="SurchargeRate"'. (in_array('SurchargeRate',$Errors) ? 'class="inputerror"' : '' ) .
		' value="'. $_POST['SurchargeRate'] .'" size=35 maxlength=40></td></tr>';
        
        /* URL for Shipper website */
        echo '<tr><td>'. _('Website (URL)') .':</td>
	<td><input type="Text" name="ShipperWebsite"'. (in_array('ShipperWebsite',$Errors) ? 'class="inputerror"' : '' ) .
	' value="'. $_POST['ShipperWebsite'] .'" size=35 maxlength=350></td></tr>            
        </table>
