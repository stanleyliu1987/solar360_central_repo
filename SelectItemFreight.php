...
if(isset($_GET['ItemFreight'])){
    $_SESSION['ItemFreight']=$_GET['ItemFreight'];
}
...
/*need to look up customer name from debtors master then populate the line items array with the sales order details records */
			...
			$LineItemsSQL = "SELECT salesorderdetails.orderlineno,
									...
									ON salesorderdetails.stkcode = stockmaster.stockid
                                                                        INNER JOIN freightcostevaluation on freightcostevaluation.salesorder =salesorderdetails.orderno AND
...
/* Only Works on From Area Selected 
 * Instead of Customer chosen
 * as Default
 */

// everything below here only do if a customer is selected

/* Set default Customer and Location for adding item line */
$_SESSION['Items'.$identifier]->Location='001';
$_SESSION['Items'.$identifier]->DebtorNo='C0001';
$_SESSION['Items'.$identifier]->Branch='B0001';
 	...
 	if (isset($_POST['CancelOrder'])) {
		...
		echo '<meta http-equiv="Refresh" content="0; url=' . $rootpath . '/CostETACalculation.php?' . SID .'identifier='.$identifier . '&pre='._('newsalesorder').'">';
		...
		echo '<p class="page_title_text"><img src="'.$rootpath.'/css/'.$theme.'/images/inventory.png" title="' . _('Order') . '" alt="" />' . ' ';
                echo _('Quotation for customer') . ' '.'<br /><br />';
	...
	/* If enter is pressed on the quick entry screen, the default button may be Recalculate */
	 if (isset($_POST['order_items'])
			...
			OR isset($_POST['Recalculate'])
                    ...
                    $_SESSION['Items'.$identifier]->DelAdd2 = $_POST['suburbSender'];
                    $_SESSION['Items'.$identifier]->DelAdd4 =  $_POST['postCodeSender'];
                    $_SESSION['Items'.$identifier]->DelAdd3 = $_POST['stateSender'];
		 ...
		 $i=1;
		  while ($i<=$_SESSION['QuickEntries'] and isset($_POST['part_' . $i]) and $_POST['part_' . $i]!='') {
			...
			$QuickEntryItemDue = 'itemdue_' . $i;
                        $QuickFreightCost='FreightCost_'.$i;
			$i++;
			...
			if (isset($_POST[$QuickEntryPOLine])) {
				...
				$NewPOLine = 0;
                        ...
                        if (isset($_POST[$QuickFreightCost])) {
				$NewFreightCost = $_POST[$QuickFreightCost];
			} else {
				$NewFreightCost = 0;
			}
			...
			if (isset($_POST['Quantity_' . $OrderLine->LineNumber])){
				...
				if ($Quantity<0 OR $Price <0 OR $DiscountPercentage >100 OR $DiscountPercentage <0){
					...
					$_SESSION['Items'.$identifier]->update_cart_item($OrderLine->LineNumber,
                        ...
                        /**Return the Total amount of Freight 2012/03/09**/
                        if(isset($_POST['FreightCost_' . $OrderLine->LineNumber])){
                         
                            $FreightTotal+=$_POST['FreightCost_' . $OrderLine->LineNumber];
                        }
                        else{
                             $FreightTotal+=$_SESSION['Items'.$identifier]->LineItems[$OrderLine->LineNumber]->FreightCost;
                        }
		}
	}
		...
		foreach($NewItem_array as $NewItem => $NewItemQty) {
				if($NewItemQty > 0)	{
					...
					$Discount = 0; /*By default - can change later or discount category override */
				        ...
				        if ($myrow=DB_fetch_array($KitResult)){
						if ($myrow['mbflag']=='K'){	/*It is a kit set item */
							$sql = "SELECT bom.parent,bom.component,
														bom.quantity
											FROM bom
											WHERE bom.parent='" . $NewItem . "'";
										//	AND bom.effectiveto > '" . Date('Y-m-d') . "'
...
//       $FreightFile='http://'.$_SERVER['HTTP_HOST'].$rootpath.'/eservice/SearchCustArea.php?CustSuburb='.str_replace(" ","_",$_SESSION['Items'.$identifier]->DelAdd2).
//                     '&CustPostCode='.$_SESSION['Items'.$identifier]->DelAdd4.'&CustState='.$_SESSION['Items'.$identifier]->DelAdd3;
//       echo $FreightFile;
//       $_SESSION['CustState']='test';
       include('eservice/SearchCustArea.php');
       
            /* Build Cost Estimation Function */
          
               $EstimateFreightCostButton='<input type=submit name=freightcost value="Estimate Freight"/>';
	       echo '<br />
		...
		foreach ($_SESSION['Items'.$identifier]->LineItems as $OrderLine) {
			...
			echo '<td><a href="' . $_SERVER['PHP_SELF'] . '?identifier=' . $identifier . '&Delete=' . $OrderLine->LineNumber . '" onclick="return confirm(\'' . _('Are You Sure?') . '\');">' . $RemTxt . '</a></td>';
                        
                      //  echo '<td><input type=button name="FreightCalculation_' . $OrderLine->LineNumber . '" value="' . _('Calculation') . '" onclick="popupFreightwindow('.$OrderLine->LineNumber.');"></td>';          
                        if(isset($_GET["freightcost"]) and ($_GET["freightline"]==$OrderLine->LineNumber)){
                            $freightcost=$_GET["freightcost"];
                        }
                        else{
                            $freightcost=$_SESSION['Items'.$identifier]->LineItems[$OrderLine->LineNumber]->FreightCost;
                        }
                      ...
                      //  echo '<tr><td colspan=10>' . _('Kit Code') . ':<input type=text name="stock" value="' . $OrderLine->StockParent . '"></td></tr>';
                   
			$_SESSION['Items'.$identifier]->total = $_SESSION['Items'.$identifier]->total + $LineTotal;
//                        if(!isset($_SESSION['Items'.$identifier]->LineItems[$OrderLine->LineNumber]->ItemVolume) and !isset($_SESSION['Items'.$identifier]->LineItems[$OrderLine->LineNumber]->ItemWeight)){
...
//                        }
//                        else{
//                        $_SESSION['Items'.$identifier]->totalVolume += $_SESSION['Items'.$identifier]->LineItems[$OrderLine->LineNumber]->ItemVolume;
//			$_SESSION['Items'.$identifier]->totalWeight += $_SESSION['Items'.$identifier]->LineItems[$OrderLine->LineNumber]->ItemWeight;   
//                        }
                        
                        $_SESSION['Items'.$identifier]->totalVolume +=  $OrderLine->Quantity * $OrderLine->Volume;
			$_SESSION['Items'.$identifier]->totalWeight +=  $OrderLine->Quantity * $OrderLine->Weight;

		} /* end of loop around items */

		$DisplayTotal = number_format($_SESSION['Items'.$identifier]->total,$_SESSION['Items'.$identifier]->CurrDecimalPlaces);
                $DisplayTotalFreight=number_format($FreightTotal,$_SESSION['Items'.$identifier]->CurrDecimalPlaces);
		if (in_array(2,$_SESSION['AllowedPageSecurityTokens'])){
			$ColSpanNumber = 3;
		} else {
			$ColSpanNumber = 1;
		...
		echo '<tr class="EvenTableRows"><td class="number" colspan=7><b>' . _('TOTAL Excl Tax/Freight') . '</b></td>
							<td colspan="' . $ColSpanNumber . '" class=number>' . $DisplayTotal . '</td>
                                                        <td class="number"><b>' . _('TOTAL Freight') . '</b></td>
							<td class=number>'.$DisplayTotalFreight.'</td></tr></table>';
                echo '<input type=hidden name=FreightTotal value="'.$DisplayTotalFreight.'" >';
		...
		echo '<br /><div class="centre"><input type=submit name="Recalculate" Value="' . _('Re-Calculate') . '"></div><hr>';
	} # end of if lines
