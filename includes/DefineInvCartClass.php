...
Class Cart {
	...
	function add_to_cart($StockID,
		...
		if (isset($StockID) AND $StockID!=""  AND isset($Qty)){
			...
			if ($UpdateDB=='Yes'){
				...
				global $db;
				$sql = "INSERT INTO stockmoves (orderlineno,
								orderno,
								stkcode,
								quantity,
								unitprice,
	...
	function remove_from_cart($LineNumber, $UpdateDB='No'){
		...
		if ($UpdateDB=='Yes'){
			...
			if ($this->Some_Already_Delivered($LineNumber)==0){
				...
				$result = DB_query("update stockmoves  set delflag=1
									WHERE stkmoveno='" . $LineNumber . "'",
									$db,
	...
	function GetTaxes($LineNumber){
        ...
        function GetNewItemTaxes($TaxCategory,$TG,$TXP){
		...
		$SQL = "SELECT taxgrouptaxes.calculationorder,
			...
			FROM taxauthrates INNER JOIN taxgrouptaxes ON
				...
				INNER JOIN taxauthorities ON
				taxauthrates.taxauthority=taxauthorities.taxid
			WHERE taxgrouptaxes.taxgroupid=" . $TG . "
			AND taxauthrates.dispatchtaxprovince=" . $TXP . "
		...
		if (DB_num_rows($GetTaxRatesResult)==0){
			...
			$myrow = DB_fetch_array($GetTaxRatesResult);
			
                        return $myrow['taxrate'];
														
			} //end loop around different taxes
		} //end if there are some taxes defined
	} //end method GetTaxes
        
        function GetAddTaxes($taxcategory){
		...
		$SQL = "SELECT 
					taxauthrates.taxrate
			FROM taxauthrates 
			WHERE taxgrouptaxes.taxcatid='" . $taxcategory . "' limit 1";
