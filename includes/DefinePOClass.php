<?php
/* $Id: DefinePOClass.php 4572 2011-05-23 10:14:06Z daintree $ */
...
/* PurchOrderDetails */
   ...
   function ConvertSOtoInvNo($salesNO,$db,$rootpath){
         $SQL = "SELECT transno FROM debtortrans WHERE type=10
							AND order_ ='" . $salesNO . "'";
          $ErrMsg =  _('The order cannot be retrieved because');
	  $DbgMsg =  _('The SQL statement that was used and failed was');
	  $GetInvNoResult = DB_query($SQL,$db,$ErrMsg,$DbgMsg);
          $myrow = DB_fetch_array($GetInvNoResult);
      
     if($myrow[0]==0){
          prnMsg ('No Invoice Number Matched, Update failed');
          echo '<meta http-equiv="Refresh" content="0; url=' . $rootpath . '/PO_SelectOSPurchOrder.php">';
          exit;
      }
      else{
          return $myrow[0];
   ...
   function ConvertInvNotoInvSO($invNO,$db,$rootpath){
         $SQL = "SELECT count(*) FROM debtortrans WHERE type=10
							AND order_ ='" . $invNO . "'";
          $ErrMsg =  _('The order cannot be retrieved because');
	  $DbgMsg =  _('The SQL statement that was used and failed was');
	  $GetInvNoResult = DB_query($SQL,$db,$ErrMsg,$DbgMsg);
          $myrow = DB_fetch_array($GetInvNoResult);
      if($myrow[0]==0){
          //prnMsg ('No Sales Order Number Matched, Update failed');
          //echo '<meta http-equiv="Refresh" content="0; url=' . $rootpath . '/PO_SelectOSPurchOrder.php">';
          //exit;
          return null;
      }
