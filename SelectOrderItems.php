	...
	if (isset($result_CustSelect)) {
		...
		while ($myrow=DB_fetch_array($result_CustSelect)) {
			...
			echo '<input type="hidden" name="FormID" value="' . $_SESSION['FormID'] . '" />';
                        echo '<td><input tabindex='.($j+5).' type=submit name="SubmitCustomerSelection' . $j .'" value="' . $myrow['debtorno'].' '.$myrow['branchcode']. '"></td>';
		...
		if ($OK_to_delete==1){
			if($_SESSION['ExistingOrder'.$identifier]!=0){
				...
				$DelResult=DB_query($SQL,$db,$ErrMsg);
                                ...
                                $SQL = "DELETE FROM freightcostevaluation WHERE salesorder='" . $_SESSION['ExistingOrder'.$identifier] . "'";
				$ErrMsg = _('The freight cost evaluation data could not be deleted because');
				$DelResult=DB_query($SQL,$db,$ErrMsg);
               			$_SESSION['ExistingOrder'.$identifier]=0;
			}
...
//            $EstimateFreightCostButton='<input type=button name=freightcost value="Estimate Freight" onclick=
//            "javascript:popupFreightwindow('.'\''.$theme.'\', '.'\''.$rootpath.'\')">';
