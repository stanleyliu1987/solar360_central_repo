...
if ((isset($_POST['UpdateStatus']) AND $_POST['UpdateStatus']!='') ) {
	...
	if($_SESSION['ExistingOrder']==0){
		...
		if ($_POST['Status'] == 'Pending' or $_POST['Status']=='BackOrder') {
			
			if($_SESSION['PO'.$identifier]->Any_Already_Received()==1){
				$OKToUpdateStatus =0; //not OK to update status
				prnMsg( _('This order could not have the status changed back to pending or back order because some of it has already been received. Quantities received will need to be returned to change the order back to pending.'),'warn');
			}
			...
			if (($AuthorityLevel>$OrderTotal OR $_SESSION['UserID']==$_SESSION['PO'.$identifier]->Initiator ) AND $OKToUpdateStatus==1) {
				...
				$_SESSION['PO'.$identifier]->StatusComments = date($_SESSION['DefaultDateFormat']).' - ' . _('Order set to pending status by') . $UserChangedStatus  . $_POST['StatusComments']. '<br />' .html_entity_decode($_POST['StatusCommentsComplete'],ENT_QUOTES,'UTF-8');
                                ...
                                $_SESSION['PO'.$identifier]->StatusComments = date($_SESSION['DefaultDateFormat']).' - ' . _('Order set to back order status by') . $UserChangedStatus  . $_POST['StatusComments']. '<br />' .html_entity_decode($_POST['StatusCommentsComplete'],ENT_QUOTES,'UTF-8');    
...
if (isset($_GET['NewOrder']) AND isset($_GET['StockID']) AND isset($_GET['SelectedSupplier'])) {
		...
		*/
		...
		$_POST['Select'] = $_GET['SelectedSupplier'];
                $_SESSION['PO'.$identifier]->Invoiceorder = $_POST['InvRefNo'];
 ...
 *  ensure session variables updated then meta refresh to PO_Items.php*/
	...
	$_SESSION['PO'.$identifier]->Port = $_POST['Port'];
        $_SESSION['PO'.$identifier]->Invoiceorder = $_POST['InvRefNo'];
        $_SESSION['PO'.$identifier]->SupWarehouseNo=$_POST['SupplierWarehouse'];
...
if((!isset($_POST['SearchSuppliers']) or $_POST['SearchSuppliers']=='' ) AND
	...
	$_POST['DeliveryDate']=$_SESSION['PO'.$identifier]->DeliveryDate;
        $_POST['InvoiceOrder']=$_SESSION['PO'.$identifier]->Invoiceorder;
        $_POST['SuppTel']=$_SESSION['PO'.$identifier]->SuppTel;
}
...
/*Seach Warehouse Address */
if (isset($_POST['SearchWarehouses'])){ 

    if (strlen($_POST['SupplierWarehouse'])>0) {
		//insert wildcard characters in spaces
		$sql = "SELECT warehousecode,
                       ...
                       email,
                       comment 
                       FROM  supplierwarehouse where warehousecode='".$_POST['SupplierWarehouse']."' and supplierid='".$_POST["Supplierid"]."'";
                $ErrMsg = _('The searched supplier records requested cannot be retrieved because');
		$result_SuppWarSelect = DB_query($sql,$db,$ErrMsg);
		
		if (DB_num_rows($result_SuppWarSelect)==1){
		$myrow=DB_fetch_array($result_SuppWarSelect);
		$_POST['SuppDelAdd1'] = $myrow['whaddress1'];
		$_POST['SuppDelAdd2'] = $myrow['whaddress2'];
		$_POST['SuppDelAdd3'] = $myrow['whaddress3'];
		$_POST['SuppDelAdd4'] = $myrow['whaddress4'];
		$_POST['SuppDelAdd5'] = $myrow['whaddress5'];
		$_POST['SuppDelAdd6'] = $myrow['whaddress6'];
		$_POST['SuppTel'] = $myrow['phoneno'];
		} 
	
            
		} elseif (DB_num_rows($result_SuppSelect)==0){
			prnMsg( _('No supplier Warehouse records contain the selected text') . ' - ' .
				_('please alter your search criteria and try again'),'info');
		}
} /*End Seach Warehouse Address */
// part of step 1
...
//sub table starts
	...
	echo '<tr><td>' . _('Initiated By') . ':</td>
        ...
        echo '<tr><td>' . _('Invoice Ref No') . ':</td>
			<td><input type="text" name="InvRefNo" size="16" maxlength="15" value="' . $_POST['InvoiceOrder'] . '" /></td></tr>';
			...
			case 'Pending':
				echo '<option selected value="Pending">' . _('Pending') . '</option>
						<option value="Authorised">' . _('Authorised') . '</option>
                                                <option value="BackOrder">' . _('Back Order') . '</option>    
...
/*  Customer deliver address modified by Stan 2011/07/19 */
	echo '<table width=80%>
		<tr>
...
//		</tr>';
        ...
        echo  '<table class=selection width=100%><tr><td></td></tr>';
	...
	while ($LocnRow=DB_fetch_array($LocnResult)){
		if (isset($_POST['StkLocation']) and ($_POST['StkLocation'] == $LocnRow['loccode'] OR
				($_POST['StkLocation']=='' AND $LocnRow['loccode']==$_SESSION['UserStockLocation']))){
			echo '<input type=hidden name=StkLocation value="'.$LocnRow['loccode'].'"/>';
		} else {
			echo '<input type=hidden name=StkLocation value="'.$LocnRow['loccode'].'"/>';
		}
	}
...
//			$_SESSION['PO'.$identifier]->SupplierContact= $_POST['SupplierContact'];
	...
	echo '</select> ';
	echo '</td></tr><tr><td>' . _('Supplier Warehouse') . ':</td><td>
		<select name="SupplierWarehouse"  onchange="ReloadForm(form1.SearchWarehouses);">';

	$sql = "SELECT warehousecode, warehousename FROM supplierwarehouse WHERE supplierid='" . $_POST['Select'] ."'";
	$SuppWhResult = DB_query($sql,$db);
        if($_SESSION['PO'.$identifier]->SupWarehouseNo==''){
         echo '<option value="">' . _('Please choose a warehouse') . '</option>';
         }
	while ( $SuppWhRow=DB_fetch_array($SuppWhResult)){
		if ($_POST['SupplierWarehouse'] == $SuppWhRow['warehousecode'] OR ($_POST['SupplierWarehouse']==''
			AND $SuppWhRow['warehousecode']==$_SESSION['PO'.$identifier]->SupWarehouseNo)){

			echo '<option selected value="' . $SuppWhRow['warehousecode'] . '">' . $SuppWhRow['warehousename'] . '</option>';
		} 
                else {
			echo '<option value="' . $SuppWhRow['warehousecode'] . '">' . $SuppWhRow['warehousename'] . '</option>';
		}
	}

	echo '</select> ';
        echo '<input type="hidden" name="Supplierid" value="' . $_POST['Select'] .'"/>';
        echo '<input type="submit" name="SearchWarehouses" value="' . _('Select') . '"></td></tr>';
	echo '</td></tr>';
