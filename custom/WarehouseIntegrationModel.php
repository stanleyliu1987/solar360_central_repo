...
while ($item = DB_fetch_array($result)) {
    $export_data.=$item['PH'].str_repeat("^", 3).$item['Customer #'].str_repeat("^", 1).$item['Order Number'].str_repeat("^", 7).
                  ...
                  str_repeat("^", 8).date("Ymd", strtotime($item["Ship Date Required"])).str_repeat("^", 7).date("Ymd").str_repeat("^", 6)."SLR".str_repeat("^", 1).preg_replace('/\s+/', ' ', trim( $item["Special Instructions 1"])).str_repeat("^", 38).
    ...
    $export_data.=$item_PD['PD'].str_repeat("^", 1)."R".str_repeat("^", 1)."ROW".str_repeat("^", 2).$item_PD['Order Number'].str_repeat("^", 7).$item_PD['Product Code'].str_repeat("^", 2).preg_replace('/\s+/', ' ', trim($this->stripcomma($item_PD['Description']))).str_repeat("^", 4).
